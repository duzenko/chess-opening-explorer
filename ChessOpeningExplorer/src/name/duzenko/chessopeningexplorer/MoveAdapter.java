...
class ChessMove {
	String move = "";
	int stat[] = {0, 0, 0}, fileRecNo;
}

public class MoveAdapter extends ArrayAdapter<ChessMove> {
	...
	void sort() {
		for(int i=0; i<getCount()-1; i++) 
			for(int j=getCount()-1; j>i; j--) {
				ChessMove m1 = getItem(j-1), m2 = getItem(j);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
    	...
    	ChessMove chessMove = getItem(position); 
